...
- **Effects not stopped when scope stops** - `watchEffect` and `watch` now properly register their effects with the current effect scope via `registerWithScope()`. Effects are now correctly stopped when their owning scope is stopped, preventing resource leaks and duplicate timers in Flutter apps.
